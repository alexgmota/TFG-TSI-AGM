...
class BasicUformerLayerWithPooling(nn.Module):
    def __init__(self, dim, output_dim, input_resolution, depth, num_heads, win_size,
        ...
        if shift_flag:
            self.blocks = nn.ModuleList([
                LeWinTransformerBlockPooling(dim=dim, input_resolution=input_resolution,
                                    num_heads=num_heads, win_size=win_size,
                                    shift_size=0 if (i % 2 == 0) else win_size // 2,
                                    mlp_ratio=mlp_ratio,
                                    qkv_bias=qkv_bias, qk_scale=qk_scale,
...
#########################################
#### LeWinTransformer with Pooling   ####
class LeWinTransformerBlockPooling(nn.Module):
    def __init__(self, dim, input_resolution, num_heads, win_size=8, shift_size=0,
                 mlp_ratio=4., qkv_bias=True, qk_scale=None, drop=0., attn_drop=0., drop_path=0.,
                 act_layer=nn.GELU, norm_layer=nn.LayerNorm,token_projection='linear',token_mlp='leff',
...
class Uformer(nn.Module):
    def __init__(self, img_size=256, in_chans=13, dd_in=3,
        ...
        self.encoderlayer_2 = BasicUformerLayerWithPooling(dim=embed_dim*4,
        ...
        self.encoderlayer_3 = BasicUformerLayerWithPooling(dim=embed_dim*8,
        ...
        # Bottleneck
        self.conv = BasicUformerLayerWithPooling(dim=embed_dim*16,
        ...
        self.decoderlayer_0 = BasicUformerLayerWithPooling(dim=embed_dim*16,
                            output_dim=embed_dim*16,
                            input_resolution=(img_size // (2 ** 3),
                                                img_size // (2 ** 3)),
                            depth=depths[5],
        ...
        self.decoderlayer_1 = BasicUformerLayerWithPooling(dim=embed_dim*8,
                            output_dim=embed_dim*8,
                            input_resolution=(img_size // (2 ** 2),
                                                img_size // (2 ** 2)),
                            depth=depths[6],
